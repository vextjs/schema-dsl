...
class DslAdapter {
  ...
  static _parseType(dsl) {
    // console.error('DEBUG _parseType ENTRY:', dsl);
    // 处理数组DSL语法
    ...
    if (constraint) {
      if (constraint.includes('|')) {
         // console.error('DEBUG FORCING ENUM');
         // Object.assign(baseSchema, { enum: constraint.split('|') });
      }
  ...
  static _parseConstraint(baseType, constraint) {
    ...
    console.error('DEBUG _parseConstraint:', constraint);
