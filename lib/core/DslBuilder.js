...
class DslBuilder {
  ...
  static validateNestingDepth(schema, maxDepth = 3) {
    ...
    function traverse(obj, depth = 0, path = '', isRoot = false) {
      if (obj && typeof obj === 'object') {
        if (obj.properties) {
          // 如果是根节点，不增加深度，直接遍历 properties
          const currentDepth = isRoot ? depth : depth + 1;

          if (!isRoot && currentDepth > maxFound) {
            maxFound = currentDepth;
            deepestPath = path;
          }

          Object.keys(obj.properties).forEach(key => {
            traverse(obj.properties[key], currentDepth, `${path}.${key}`.replace(/^\./, ''), false);
          });
        }
        if (obj.items) {
          // 数组items不增加深度
  ...
  username(preset = 'medium') {
    ...
    return this.pattern(new RegExp(pattern))
      .messages({
        'pattern': '用户名必须以字母开头，只能包含字母、数字和下划线'
      });
  }
  ...
  password(strength = 'medium') {
    ...
    return this.pattern(pattern)
      .messages({
        'pattern': `密码至少${minLengths[strength]}位${strength === 'weak' ? '' : '，需包含字母和数字'}`
      });
  }
  ...
  phone(country = 'cn') {
    ...
    const configs = {
      cn: { pattern: /^1[3-9]\d{9}$/, min: 11, max: 11, msg: '请输入有效的手机号' },
      us: { pattern: /^\d{10}$/, min: 10, max: 10, msg: '请输入有效的手机号' },
      uk: { pattern: /^(\+44\s?)?0?\d{10}$/, min: 10, max: 15, msg: '请输入有效的手机号' },
      hk: { pattern: /^[5-9]\d{7}$/, min: 8, max: 8, msg: '请输入有效的手机号' },
      tw: { pattern: /^09\d{8}$/, min: 10, max: 10, msg: '请输入有效的手机号' },
      international: { pattern: /^\+[1-9]\d{1,14}$/, min: 8, max: 15, msg: '请输入有效的国际手机号' }
    };
    ...
    return this.pattern(config.pattern).messages({ 'pattern': config.msg });
  }
}
