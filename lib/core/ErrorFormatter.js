...
class ErrorFormatter {
  ...
  formatDetailed(errors, locale) {
    ...
    return errors.map(err => {
      ...
      const type = schema.type || 'string'; // 默认为 string

      // 查找自定义消息
      // 优先级:
      // 1. type.keyword (如 string.pattern)
      // 2. mappedKeyword (如 min)
      // 3. keyword (如 minLength)
      // 4. default
      let message = customMessages[`${type}.${mappedKeyword}`] ||
                    customMessages[`${type}.${keyword}`] ||
      ...
      if (!message) {
        ...
        if (typeof message === 'string' && (message.includes('.') || messages[message])) {
          if (message === 'pattern.objectId' && !messages[message]) {
             console.error('DEBUG: pattern.objectId missing in messages. Locale:', this.locale);
             console.error('DEBUG: messages keys:', Object.keys(messages));
          }
