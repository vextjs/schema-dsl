...
class CustomKeywords {
  ...
  static registerMetadataKeywords(ajv) {
    console.log('Registering metadata keywords...');
    // _label: 字段标签
    ajv.addKeyword({
      keyword: '_label',
        ...
        for (const validator of validators) {
          try {
            ...
            if (result instanceof Promise) {
              ...
              throw new Error('Async validation not supported in synchronous validate()');
            }

            // 处理返回值
            if (result === false) {
              validate.errors = [{ message: 'Validation failed' }];
              return false;
            }
            ...
            if (result && typeof result === 'object' && result.error) {
              validate.errors = [{ message: result.message || 'Validation failed' }];
              return false;
            }
          } catch (error) {
