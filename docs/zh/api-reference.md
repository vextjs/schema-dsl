...
DSL 主入口命名空间。`s` 和 `dsl` 是同一个函数对象：`s === dsl`。函数调用形式支持字符串和对象两种定义方式；命名空间 factory 提供可发现的链式入口，但底层仍复用同一套 DSL 解析与 builder 契约。v2.1.0 起，公开示例默认使用 `schema-dsl/pure` + `s`，避免导入时修改全局原型。
...
| 扩展辅助 | `s.defineExtension(definition)`、`s.registerExtension(definition)` |
...
为有意使用 `'string!'.description(...)` 这类直接字符串链式写法的项目安装或重新安装 String 扩展。普通 v2.1.0 文档示例不需要这个 API，因为默认从 `schema-dsl/pure` + `s` 开始。
...
| `runtime.registerExtension(definition)` | 注册 runtime 作用域内的自定义 DSL 扩展和可选 namespace factory。 |
