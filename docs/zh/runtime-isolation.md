...
使用 `clearCache()` 清理 runtime 持有的 Validator/AJV 缓存，使用 `getStats()` 查看 message/type/pattern/cache 统计，在应用关闭、插件卸载或测试收尾时调用 `dispose()`。`dispose()` 可重复调用；dispose 后继续使用会抛出 `[schema-dsl/runtime] Runtime has been disposed`。
...
`runtime.dsl('string')` 和 `runtime.compileField('string')` 返回与普通 `dsl('string')` 相同的链式 builder 形态，因此内建链式方法保留已有 TypeScript 提示。

自定义 runtime DSL 类型通过 `types`、`dynamicTypes` 或 `typeResolver` 传给 `createRuntime()`。自定义链式方法仍通过 builder 接口的 TypeScript module augmentation 提供类型声明，并在扩展代码里提供对应运行时方法。
...
provider 覆盖标准 AJV 错误表、custom keyword、条件校验、异步 custom validator fallback 消息，以及 runtime 创建的 `I18nError`。显式传入的 `messages` 优先级仍高于 provider fallback。

## 什么时候不需要

如果需要 v1 兼容的自动 String 链式安装，使用 root `schema-dsl`。如果只需要避免原型污染，不需要隔离运行时状态，使用 `schema-dsl/pure`。
