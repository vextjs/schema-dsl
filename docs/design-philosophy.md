...
| 场景 | Schema-DSL | Zod | Ajv (raw) | Joi |
|------|-----------|-----|-----------|-----|
| S1 简单有效 | **1.301M ops/s** | 1.305M ops/s | 4.732M ops/s | 154K ops/s |
| S2 无效（均无 i18n）| **1.205M ops/s** | 13.49K ops/s | 4.874M ops/s | 92K ops/s |
| S3 嵌套有效 | **1.085M ops/s** | 847K ops/s | 3.974M ops/s | 125K ops/s |
...
- **有效数据场景（S1）**：schema-dsl 与 Zod 持平（相差 <1%）；**S3 嵌套场景**快约 **28%**
...
|------|-----------|-----|-----|-----|
| **性能** | ✅ 1.657M ops/s（S1有效） | 1.308M ops/s | 🥇 5.579M ops/s | 174K ops/s |
...
优势：
  ...
  ✅ 功能最全（独家：DB导出、用户语言包）
  ✅ 易用性最佳（5分钟上手）
  ✅ 性能优秀（S3 嵌套快于 Zod 28%，S1 持平；无效数据公平对比快 89x）

权衡：
  ⚠️ 比 Ajv (raw) 慢约 4x（DSL 层自身开销：DSL 解析 + coerce + 缓存）
  ⚠️ TypeScript 类型推断有限
